...
**Latest release:** v0.5.4 — OpenAI-compatible local-server UX overhaul + Gemini list refresh. The Model field in the API Keys screen is now an auto-populated dropdown instead of free-text — click "Test & Load Models" to fetch what's loaded on your local server, then pick from the menu. Status pill ("LM Studio · qwen3-coder-30b ready") only goes green when the configured model is actually loaded, catching typos before save. Gemini list adds `gemini-3-pro-preview` and `gemini-3.1-flash-lite-preview`, drops `gemini-2.0-flash` and `gemini-2.5-flash-lite`. CFBundleVersion `1777000002`. Builds on v0.5.3's GPT-5.5 / Claude Opus 4.7 / Grok 4.20 default refresh and v0.5.2's Sparkle build-number scheme lock.
