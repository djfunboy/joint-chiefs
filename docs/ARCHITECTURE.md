...
- **Release artifact:** notarized + stapled DMG containing `Joint Chiefs.app` with the four binaries in `Contents/Resources/` (CLI/MCP/keygetter) and `Contents/MacOS/jointchiefs-setup`. Shipped through v0.5.4; SHA-256 wired into `Casks/joint-chiefs.rb`.
- **Sparkle appcast** at [jointchiefs.ai/appcast.xml](https://jointchiefs.ai/appcast.xml) — EdDSA-signed entries for v0.5.0, v0.5.2, v0.5.3, and v0.5.4. Pre-v0.5.0 entries were stripped after the v0.5.0 build-number bug (see `tasks/lessons.md` 2026-04-26 — they used Unix-timestamp `CFBundleVersion` values that exceeded v0.5.0's sequential `5`, causing Sparkle to "downgrade" fresh installs).
