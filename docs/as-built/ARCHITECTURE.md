...
- **Release artifact:** notarized + stapled DMG containing `Joint Chiefs.app` with the four binaries in `Contents/Resources/` (CLI/MCP/keygetter) and `Contents/MacOS/jointchiefs-setup`. Shipped through v0.5.13; SHA-256 wired into `Casks/joint-chiefs.rb`.
- **Sparkle appcast** at [jointchiefs.ai/appcast.xml](https://jointchiefs.ai/appcast.xml) — one EdDSA-signed `<item>` per shipped release, newest first, back to v0.5.0. The feed is the release gate's source of truth for what users can actually update to; a tagged release that never lands an entry here is invisible to installed copies. Pre-v0.5.0 entries were stripped after the v0.5.0 build-number bug (see the [build-number rule](../../tasks/lessons.md) — they used Unix-timestamp `CFBundleVersion` values that exceeded v0.5.0's sequential `5`, causing Sparkle to "downgrade" fresh installs).
