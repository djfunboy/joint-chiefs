...
- [x] Bundled in `Joint Chiefs.app` with `Contents/Resources/` binaries — DMGs notarized + stapled through v0.5.13
...
- [x] Pre-flight validation: inline error plus disabled Save when the moderator (or a specific tiebreaker) is a provider with no usable API key
...
1. **Summary out, transcript on disk.** The CLI/API returns only the consensus summary. Full debate transcripts are written to local files for replay or pipeline inspection; a UI viewer remains deferred.
2. **Universal trigger.** Any MCP-aware client can spawn the stdio server, and any tool that can run a shell command can invoke the CLI. No HTTP server or client lock-in.
...
| 1.8 | 2026-05-27 | Corrected design principles to match the shipping stdio-only MCP architecture: no HTTP server trigger path, no localhost-server privacy claim, and transcripts live on disk until the deferred UI viewer exists. |
| 1.9 | 2026-08-02 | F6 moderator key pre-flight checked off — inline error plus disabled Save when the moderator or a specific tiebreaker has no usable key. Scope covers the tiebreaker picker, which had the same defect. |
